...
export default function AgritechIndustry() {
  const title = 'Agritech & Precision Farming | Diwave Industries';
  const description =
    'Дрони та сенсори, NDVI/карти полів, аналітика вологості/живлення, планування внесень.';
  const url = `${baseUrl}/industries/agritech`;

  const jsonLd = [{ '@context': 'https://schema.org', '@type': 'CollectionPage', name: title, url }];

  return (
    <>
      <SEO title={title} description={description} url={url} image={`${baseUrl}/images/industries/agritech/cover.jpg`} locale="uk" jsonLd={jsonLd} />
      <main>
        <section className="mx-auto max-w-7xl px-6 pt-16">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold text-white tracking-tight">Agritech & Precision Farming</h1>
