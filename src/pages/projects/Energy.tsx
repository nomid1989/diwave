...
export default function EnergyProject() {
  const title = 'Енергомоніторинг — кейс | Diwave Solutions';
  const description =
    'Smart meters, попередження та рекомендації. Дашборди енергоспоживання для економії та контролю.';
  const url = `${baseUrl}/projects/energy`;

  const jsonLd = [
    {
      '@context': 'https://schema.org',
      '@type': 'CreativeWork',
      name: 'Кейс: Енергомоніторинг',
      description:
        'Впровадження дашбордів енергії: smart meters, попередження, рекомендації.',
  ...
  return (
    <>
      <SEO title={title} description={description} url={url} image={`${baseUrl}/images/projects/energy/cover.jpg`} locale="uk" jsonLd={jsonLd} />
      ...
      <main className="relative">
        <section className="mx-auto max-w-7xl px-6 pt-16">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold text-white tracking-tight">Енергомоніторинг</h1>
              <p className="mt-4 text-gray-300">{description}</p>
              <div className="mt-6 flex flex-wrap gap-3">
                <a href="/solutions/iot-scada" className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 bg-white/90 text-black font-medium hover:bg-white transition">Перейти до рішення</a>
            ...
            <div className="relative">
              ...
              <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur">
                <SmartImage srcFolder="/images/projects/energy" alt="Кейс: енергомоніторинг" className="w-full h-[420px]" imgClassName="w-full h-[420px] object-cover" />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-24">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <a href="/solutions/iot-scada" className="inline-flex items-center gap-2 rounded-full px-4 py-2 border border-white/20 text-white hover:bg-white/10">Пов’язане рішення</a>
          </div>
        </section>
      </main>
    </>
