...
export default function VendingProject() {
  const title = 'Вендинг: телеметрія та платежі — кейс | Diwave Solutions';
  const description =
    'KYC, безготівкові оплати, віддалений збір інкасацій, маршрути. Прозора телеметрія, контроль, оптимізація логістики.';
  ...
  const jsonLd = [
    {
      ...
      '@type': 'CreativeWork',
      name: 'Кейс: Вендинг — телеметрія та платежі',
      description:
        'KYC, cashless, віддалений моніторинг та інкасації, маршрутизація та аналітика.',
  ...
  return (
    <>
      <SEO title={title} description={description} url={url} image={`${baseUrl}/images/projects/vending/cover.jpg`} locale="uk" jsonLd={jsonLd} />
      ...
      <main className="relative">
        <section className="mx-auto max-w-7xl px-6 pt-16">
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            <div>
              <h1 className="text-3xl md:text-5xl font-semibold text-white tracking-tight">Вендинг: телеметрія та платежі</h1>
              <p className="mt-4 text-gray-300">{description}</p>
              <div className="mt-6 flex flex-wrap gap-3">
                <a href="/solutions/alcohol-vending" className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 bg-white/90 text-black font-medium hover:bg-white transition">Перейти до рішення</a>
            ...
            <div className="relative">
              ...
              <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur">
                <SmartImage srcFolder="/images/projects/vending" alt="Кейс: вендинг" className="w-full h-[420px]" imgClassName="w-full h-[420px] object-cover" />
              </div>
            </div>
          </div>
        </section>
        <section id="results" className="mx-auto max-w-7xl px-6 py-12">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            <div className="mt-4 grid md:grid-cols-3 gap-4">
              {[
                { m: '↓', t: 'Витрати на обслуговування' },
                { m: '↑', t: 'Оперативність рішень' },
                { m: '↑', t: 'Контроль і прозорість' }
              ].map((r) => (
        ...
        <section className="mx-auto max-w-7xl px-6 pb-24">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <a href="/solutions/alcohol-vending" className="inline-flex items-center gap-2 rounded-full px-4 py-2 border border-white/20 text-white hover:bg-white/10">Пов’язане рішення</a>
          </div>
        </section>
      </main>
    </>
